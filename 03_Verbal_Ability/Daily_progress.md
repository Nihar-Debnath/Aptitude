## Current Progress ⬇️

---

### Verbal Ability
- **Learning Source:** https://www.youtube.com/watch?v=GDAVuWg0G8M
- **Current Timestamp:** `1:13:54`
- **Reading Compresension Timestamp:** `from 7:59:00 to 8:58:02`

##### Problem Solving Progress
1. **Chapter:** Vocabulary (one shot done ✅)
    1. **Part:** 4.1.1 Word Meanings (Contextual Vocabulary)
        - **PYQ:** `Total 0 = pending`
        - **Last Completed Practice Question:** `10`

2. **Chapter:** Sentence Correction (currently going on)
    1. **Part:** 4.13.2 Subject–Verb Agreement 
        - **PYQ:** `Total 0 = pending`
        - **Last Completed Practice Question:** `0`
    2. **Part:** 4.13.3 repetition 
        - **PYQ:** `Total 0 = pending`
        - **Last Completed Practice Question:** `0`
    3. **Part:** 4.13.4 modifiers
        - **PYQ:** `Total 0 = pending`
        - **Last Completed Practice Question:** `0`
    4. **Part:** 4.13.5 parallelism
        - **PYQ:** `Total 0 = pending`
        - **Last Completed Practice Question:** `0`

3. **Chapter:** Reading Comprehension (one shot done ✅)
    1. **Part:** 4.14.1 Basic Reading & Understanding (Literal Comprehension)
        - **PYQ:** `Total 0 = pending`
        - **Last Completed Practice Question:** `0`

3. **Chapter:** Cloze test (one shot done ✅)
        - **PYQ:** `Total 0 = pending`  
        - **Last Completed Practice Question:** `0`



# Next day, before starting the verbal ability core, must finish the basic grammer first!!!